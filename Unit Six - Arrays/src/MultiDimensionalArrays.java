public class MultiDimensionalArrays {
    ...
    private static void exampleTwo() { 
      ...
      for (int i = 0; i < arr2D.length; i++) {
        arr2D[i] = new int[i + 1]; // makes jagged array
      }
    ...
    private static void display(int[][] arr2d) {
      for (int i = 0; i < arr2d.length; i++) {
        for (int j = 0; j < arr2d[i].length; j++) {
          System.out.printf("%4d", arr2d[i][j]); // printf mean print with formatting, %4d means give space of 4 for integer
        }
        System.out.println();
      }
      System.out.println();
