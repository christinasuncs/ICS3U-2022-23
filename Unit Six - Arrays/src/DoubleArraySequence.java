...
public class DoubleArraySequence implements Cloneable {
   ...
   * Initialize an empty sequence with an initial capacity of 10. Note that
   ...
   /**
     ...
     *                current
     *                element. 
     *                If there was no current element, then the new element
     *                is placed
   ...
   public void addAll(DoubleArraySequence addend) {
      ...
      if (this.data.length < this.manyItems + addend.manyItems){
         ensureCapacity(this.manyItems + addend.manyItems);
      }
      for (int i = 0; i < addend.manyItems; i++) {
         data[this.manyItems + i] = addend.data[i];
      }
   ...
   public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
      // make copy of s1 add s2 to it, use other methods
      if (s1 == null || s2 == null){
      ...
      for (int i = 0; i < s2.manyItems; i++){
         newSequence.data[s1.manyItems+i] = s2.data[i];
      }
   ...
   public void removeCurrent() {
      // shift, manyItems--
      if (!isCurrent()){
         throw new IllegalStateException("No current element, function may not be called");
      }
